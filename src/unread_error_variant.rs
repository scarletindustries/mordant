...
rustc_session::declare_lint! {
    ...
    /// never named by a pattern outside the enum's own impls. The crate
    /// distinguishes other variants of the enum, so matching is how this enum
    /// is consumed, and this variant's information only ever reaches anyone
    /// through a catch-all or `Display`. Structure nobody reads is a string
    /// with extra steps.
...
/// True when `hir_id` sits inside an impl whose self type is `enum_did`. The
/// enum's own `Display`, `Debug`, `From` and friends must match its variants
/// to exist; only patterns elsewhere show the crate consuming the structure.
fn inside_own_impl(cx: &LateContext<'_>, hir_id: rustc_hir::HirId, enum_did: DefId) -> bool {
    let mut cur = hir_id.owner.def_id.to_def_id();
    loop {
        if matches!(cx.tcx.def_kind(cur), rustc_hir::def::DefKind::Impl { .. }) {
            let self_ty = cx
                .tcx
                .type_of(cur)
...
impl<'tcx> LateLintPass<'tcx> for UnreadErrorVariant {
    ...
    fn check_pat(&mut self, cx: &LateContext<'tcx>, pat: &'tcx Pat<'tcx>) {
        ...
        if inside_own_impl(cx, pat.hir_id, enum_did) {
            return;
        }
        self.enums
            .entry(enum_did)
    ...
    fn check_crate_post(&mut self, cx: &LateContext<'tcx>) {
        for (enum_did, facts) in &self.enums {
            // The crate must distinguish at least one variant by pattern;
            // otherwise matching is simply not how this enum is consumed, and
            ...
            for (variant, span) in &facts.constructed {
                ...
                emit(
                    ...
                    *span,
                    format!(
                        "`{}` is constructed here, but no pattern outside `{}`'s own impls ever names it",
